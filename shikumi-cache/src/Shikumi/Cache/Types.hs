...
-- The persistent backends (SQLite/Redis/Postgres — separate milestones) store
-- its JSON encoding; those add the @ToJSON@/@FromJSON@ instances for the baikai
-- 'Response' graph that baikai itself does not ship in full. Decoding an entry
-- whose 'keyVersion' does not match the live namespace is treated as a cache
-- MISS, so a version bump never serves a stale-schema row.
