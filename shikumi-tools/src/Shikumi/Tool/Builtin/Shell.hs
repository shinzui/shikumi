-- | Built-in shell tool over 'Shikumi.Tool.Env.envExec'.
--
-- Security posture: 'bashTool' executes arbitrary model-chosen shell commands
-- through the supplied t'ToolEnv', with whatever filesystem access, environment,
-- and process privileges that environment grants. With 'Shikumi.Tool.Env.localToolEnv' this is
-- deliberately non-hermetic host execution for trusted, local, single-operator
-- workflows. Sandboxing is achieved by supplying a confining t'ToolEnv', not by
-- this module.
